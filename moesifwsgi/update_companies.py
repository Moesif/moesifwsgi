...
class Company:
    ...
    def update_company(self, company_profile, api_client, DEBUG):
        if not company_profile:
            logger.info('Expecting the input to be either of the type - CompanyModel, dict or json while updating company for pid - '
            ...
            if isinstance(company_profile, dict):
                if 'company_id' in company_profile:
                    try:
                        ...
                        if DEBUG:
                            logger.info('Company Profile updated successfully for pid - ' + self.logger_helper.get_worker_pid())
                    except APIException as inst:
                        if 401 <= inst.response_code <= 403:
                            logger.error("Unauthorized access sending event to Moesif. Please check your Application Id for pid - "
                                  + self.logger_helper.get_worker_pid())
                        if DEBUG:
                            logger.info("Error while updating company, with status code for pid - " + self.logger_helper.get_worker_pid())
                            logger.info(inst.response_code)
                else:
                    logger.info('To update an company, an company_id field is required for pid - ' + self.logger_helper.get_worker_pid())
                    ...
                    try:
                        ...
                        if 401 <= inst.response_code <= 403:
                            logger.error("Unauthorized access sending event to Moesif. Please check your Application Id for pid - "
                                  + self.logger_helper.get_worker_pid())
                        if DEBUG:
                            logger.info("Error while updating company, with status code for pid - "
                                  + self.logger_helper.get_worker_pid())
                            logger.info(inst.response_code)
                else:
                    logger.info('To update a company, a company_id field is required for pid - ' + self.logger_helper.get_worker_pid())
            else:
                try:
                    ...
                    if 'company_id' in company_profile_json:
                        ...
                        logger.info('To update a company, an company_id field is required for pid - '
                              + self.logger_helper.get_worker_pid())
                except:
                    logger.warning('Error while deserializing the json, please make sure the json is valid for pid - '
                          + self.logger_helper.get_worker_pid())

    def update_companies_batch(self, company_profiles, api_client, DEBUG):
        if not company_profiles:
            logger.info('Expecting the input to be either of the type - List of CompanyModel, dict or json while updating companies for pid - '
                  + self.logger_helper.get_worker_pid())
        else:
                    ...
                    try:
                        ...
                        if 401 <= inst.response_code <= 403:
                            logger.error("Unauthorized access sending event to Moesif. Please check your Application Id.for pid - "
                                  + self.logger_helper.get_worker_pid())
                        if DEBUG:
                            logger.info("Error while updating companies, with status code for pid - "
                                  + self.logger_helper.get_worker_pid())
                            logger.info(inst.response_code)
                else:
                    logger.info('To update companies, an company_id field is required for pid - ' + self.logger_helper.get_worker_pid())
                        ...
                        try:
                            ...
                            if 401 <= inst.response_code <= 403:
                                logger.error("Unauthorized access sending event to Moesif. Please check your Application Id for pid - "
                                      + self.logger_helper.get_worker_pid())
                            if DEBUG:
                                logger.info("Error while updating companies, with status code for pid - " + self.logger_helper.get_worker_pid())
                                logger.info(inst.response_code)
                    else:
                        logger.info('To update companies, an company_id field is required for pid - '
                              + self.logger_helper.get_worker_pid())
                except:
                    logger.warning('Error while deserializing the json, please make sure the json is valid for pid - '
                          + self.logger_helper.get_worker_pid())
