...
class User:
    ...
    def update_user(self, user_profile, api_client, DEBUG):
        if not user_profile:
            logger.info('Expecting the input to be either of the type - UserModel, dict or json while updating user for pid - '
            ...
            if isinstance(user_profile, dict):
                if 'user_id' in user_profile:
                    try:
                        ...
                        if DEBUG:
                            logger.info('User Profile updated successfully for pid - ' + self.logger_helper.get_worker_pid())
                    except APIException as inst:
                        if 401 <= inst.response_code <= 403:
                            logger.error("Unauthorized access sending event to Moesif. Please check your Application Id for pid - "
                                  + self.logger_helper.get_worker_pid())
                        if DEBUG:
                            logger.info("Error while updating user, with status code for pid - " + self.logger_helper.get_worker_pid())
                            logger.info(inst.response_code)
                else:
                    logger.info('To update an user, an user_id field is required for pid - ' + self.logger_helper.get_worker_pid())
    ...
    def update_users_batch(self, user_profiles, api_client, DEBUG):
        if not user_profiles:
            logger.info('Expecting the input to be either of the type - List of UserModel, dict or json while updating users for pid - '
                  + self.logger_helper.get_worker_pid())
        else:
            if all(isinstance(user, dict) for user in user_profiles):
                    ...
                    try:
                        ...
                        if 401 <= inst.response_code <= 403:
                            logger.error("Unauthorized access sending event to Moesif. Please check your Application Id for pid - "
                                  + self.logger_helper.get_worker_pid())
                        if DEBUG:
                            logger.info("Error while updating users, with status code for pid - " + self.logger_helper.get_worker_pid())
                            logger.info(inst.response_code)
                else:
                    logger.info('To update users, an user_id field is required for pid - ' + self.logger_helper.get_worker_pid())
                ...
                try:
                    ...
                    if all(isinstance(user, dict) for user in user_profiles_json) and all(
                        ...
                        logger.info('To update users, an user_id field is required for pid - '
                              + self.logger_helper.get_worker_pid())
                except:
                    logger.warning('Error while deserializing the json, please make sure the json is valid for pid - '
                          + self.logger_helper.get_worker_pid())
